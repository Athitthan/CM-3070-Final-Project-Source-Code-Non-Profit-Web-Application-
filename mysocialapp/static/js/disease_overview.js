 ...
 searchInput.addEventListener("input", function (e) {
    ...
    debounceTimer = setTimeout(() => {
      if (query.length >= 0) {
        ...
        fetch(
          `/api/search_disease_overview/?q=${query}`
        )
          ...
          .then((data) => {
            ...
            data.forEach((item) => {
              ...
              <li class="list_title">
             <a style="text-decoration: none; color:black" href="../disease_overview/?contentId=${item.id}#information_content_container">
...
searchButton.addEventListener("click", function () {
  ...
  if(query != ""){
      ...
      window.location.href = `/disease_overview_list/?q=${encodeURIComponent(query)}#information_content_container`;
    }
});