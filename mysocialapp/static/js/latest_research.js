...
searchInput.addEventListener("input", function (e) {
  ...
  debounceTimer = setTimeout(() => {
    if (query.length >= 0) {
      ...
      fetch(`/api/search_latest_research/?q=${query}`)
        ...
        .then((data) => {
          if (data != null && data != " ") {
            ...
            data.forEach((item) => {
              ...
              <li class="list_title">
             <a style="text-decoration: none; color:black" href="../latest_research/?contentId=${item.id}#information_content_container">
...
searchButton.addEventListener("click", function () {
  ...
  if (query != "") {
    window.location.href = `/latest_research_list/?q=${encodeURIComponent(
      query
